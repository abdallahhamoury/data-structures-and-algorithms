...
const screenForNames = (arr) => {
  ...
  return arr.filter(value => (/^(Mr.||Mrs.||Ms.||Dr.\s)[A-Za-z]/.test(value)))
...
const toTitleCase = (arr) => {
  ...
  arr.map(value => {
    let newArr1 = value.substring(0, 1).toUpperCase() + value.substring(1)
...
const sortBy = (property, arr) => {
  return typeof arr[0][property] === "string"
    ? arr.sort((x, y) => x[property].localeCompare(y[property]))
    : arr.sort((x, y) => x[property] - y[property]);
};
