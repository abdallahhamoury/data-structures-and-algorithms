...
const raisedToTheThird = (arr) => {
  ...
  arr.forEach(num => {
    ...
    mathpowArr.push(Math.pow(num, 3));
...
const addOne = (arr) => {
  ...
  let addNumOne = arr.map(number => number + 1)
...
const addQuestion = (arr) => {
  let questionMark = arr.map(string => string + '?');
  return questionMark;
};
...
const forLoopTwoToThe = (arr) => {
  let arrIntegers = arr.map((num) => {
    let num1 =  Math.pow(2,num);
    return num1;
  })
return arrIntegers;
};
