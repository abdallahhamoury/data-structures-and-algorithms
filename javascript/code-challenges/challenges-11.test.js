...
function transformToLis(obj) {
  let newArr = [];
  let arrayObject = object.entries(obj);
...
const totalSum = (input) => {
  ...
  input.forEach(element => {
    ...
    Array.isArray(element) ? total += totalSum(element) :
  ...
  input.map((item, index) => {
    ...
    item.map(value => {
      if (typeof (value) === 'number' && value % 5 === 0) {
        let percentV = Math.pow(2, value);
        newArr[index].push(percentV);
      }
    })
  })
  return newArr;
...
let findMaleAndFemale = (data) => {
  ...
  data.map(element => {
    if (element.gender === 'female' || element.gender==='male'){
      arraynew.push(element.name);
    }
});
newArr = arraynew.join(' and ');
return newArr;
};
...
let findShortest = (data) => {
  let newArr = data.sort((a, b) => {
    ...
    else if (parseInt(a.height) < (b.height)) {
      return -1;
    }
    else {
      return 0;
